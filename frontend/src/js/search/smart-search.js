 ...
 * Universal search with API integration
 */
...
function initSmartSearch(inputSelector) {
  const inputs = document.querySelectorAll(inputSelector || 'input[data-search], .search-input, #heroSearchInput, #navSearchInput');

  inputs.forEach(input => {
    // Create suggestions container if not exists
    if (!input.nextElementSibling?.classList.contains('search-suggestions')) {
      const suggestionsEl = document.createElement('div');
      suggestionsEl.className = 'search-suggestions absolute z-50 bg-white rounded-xl shadow-card border border-outline mt-1 w-full max-h-64 overflow-y-auto hidden';
    ...
    input.addEventListener('input', (e) => {
      ...
      searchDebounceTimer = setTimeout(() => fetchSuggestions(query, input), 300);
    });
  ...
  suggestionsEl.innerHTML = products.map((p, index) => `
    <div class="suggestion-item flex items-center gap-3 px-4 py-3 hover:bg-surface-dim cursor-pointer transition-colors border-b border-outline last:border-b-0" 
      ...
      <div class="flex-1 min-w-0">
        <div class="text-sm font-bold text-stone-900 truncate">${highlightQuery(p.name, input.value)}</div>
        <div class="text-xs text-stone-500">${p.brand} • ${p.sku} • $${p.price?.toFixed(2)}</div>
      </div>
      ${p.stock > 0 ? '<span class="text-xs text-green-600 font-bold">En stock</span>' : '<span class="text-xs text-orange-600 font-bold">Bajo pedido</span>'}
    </div>
  `).join('');
...
function executeSearch(query, input) {
  if (query.trim()) {
    window.location.href = `/busqueda_resultados.html?q=${encodeURIComponent(query.trim())}`;
  }
}
...
function updateSuggestionHighlight(items) {
  items.forEach((item, index) => {
    item.classList.toggle('bg-surface-dim', index === selectedSuggestionIndex);
  });
}
...
window.selectSmartProduct = selectSmartProduct;
