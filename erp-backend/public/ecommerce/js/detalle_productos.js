...
function renderProduct(product) {
    ...
    if (specsTable) {
        specsTable.innerHTML = '<table class="w-full"><tbody class="text-xs font-black uppercase tracking-widest">' +
    ...
    if (compatibilityGrid) {
        compatibilityGrid.innerHTML = product.compatibility.map(c => `
    ...
    if (installationSteps) {
        installationSteps.innerHTML = product.installation.map(s => `
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    updateCartCount();
    checkFavorite(product ? product.id : null);
});
...
function checkFavorite(productId) {
    ...
    const icon = document.getElementById('favoriteIcon');
    if (icon && favorites.includes(productId)) {
