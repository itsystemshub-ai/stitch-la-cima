...
@section('title', 'MAYOR DE REPUESTO LA CIMA, C.A.')

@push('styles')
    <link rel="stylesheet" href="/frontend/public/ecommerce/css/detalle_productos.css">
@endpush
...
<main class="pt-20 pb-24">
    <section class="max-w-screen-2xl mx-auto px-6 md:px-12 py-12">
        ...
        <nav class="mb-8 flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-on-surface-variant">
            <a href="/tienda/index" class="hover:text-primary transition-colors flex items-center gap-1">
                <span class="material-symbols-outlined text-sm">home</span> Inicio
            </a>
            <span class="material-symbols-outlined text-xs">chevron_right</span>
            <a href="/tienda/catalogo_general" class="hover:text-primary transition-colors">Catálogo</a>
            <span class="material-symbols-outlined text-xs">chevron_right</span>
...
@push('scripts')
    <script>
    // Inyectar datos del producto para que el JS los use sin recargar de una DB falsa
    window.productData = {
        id: {{ $product->id }},
        name: "{{ $product->nombre }}",
        brand: "{{ $product->marca }}",
        sku: "{{ $product->codigo_oem }}",
        price: {{ $product->precio_mayor }},
        stock: {{ $product->stock_actual }},
        category: "{{ $product->categoria }}",
        images: ["{{ $product->imagen_url ?: 'https://via.placeholder.com/800x800?text=' . urlencode($product->nombre) }}"],
        description: "Repuesto de alta calidad para sistemas de {{ $product->categoria }}. Garantizado por Mayor de Repuesto La Cima.",
        technicalNote: "Este componente ha sido verificado bajo estándares OEM para asegurar máxima durabilidad en condiciones de alta exigencia.",
        specs: {
            "Marca": "{{ $product->marca }}",
            "OEM": "{{ $product->codigo_oem }}",
            "Categoría": "{{ $product->categoria }}",
            "Condición": "Nuevo"
        }
    };

    // Inyectar productos relacionados
    window.relatedProductsData = [
        @foreach($relatedProducts as $rp)
        {
            id: {{ $rp->id }},
            name: "{{ $rp->nombre }}",
            price: {{ $rp->precio_mayor }},
            category: "{{ $rp->categoria }}",
            image: "{{ $rp->imagen_url ?: 'https://via.placeholder.com/400x400?text=' . urlencode($rp->nombre) }}"
        },
        @endforeach
    ];
</script>
<script src="/frontend/public/ecommerce/js/detalle_productos.js"></script>
@endpush
