        ...
        if (window.zenithApi.isAuthenticated()) {
          try {
            ...
            if (usersResponse.status === "success" && usersResponse.data?.users) {
              console.log("Usuarios cargados desde API:", usersResponse.data.users.length);
            }
          } catch (e) {
        ...
        if (request) {
          ...
          if (window.zenithApi.isAuthenticated()) {
            try {
              const usersResponse = await window.zenithApi.getUsers();
              if (usersResponse.status === "success" && usersResponse.data?.users) {
                const user = usersResponse.data.users.find((u) => u.email === request.email);
                if (user) {
                  await window.zenithApi.resetUserPassword(user.id, request.newPassword);
                }
              }
            } catch (e) {
              console.warn("No se pudo actualizar en el backend, usando localStorage");
            }
          }
